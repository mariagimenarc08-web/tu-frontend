...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUser = async () => {
    try {
      const res = await fetch('https://tu-backend-8bl4.onrender.com/api/auth/perfil', { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  const login = async (email, password) => {
    const res = await fetch('https://tu-backend-8bl4.onrender.com/api/auth/login', {
      method: 'POST',
  ...
  const register = async (data) => {
    const res = await fetch('https://tu-backend-8bl4.onrender.com/api/auth/registro', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
